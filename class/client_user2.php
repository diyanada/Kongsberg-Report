...
class client_user2{
	...
	function client_user2($user_id, $role_id, $username){
	...
	function com_name(){
		...
		if ($conn == true){
			$sql = "SELECT [COMPANY] FROM [SYSTEMUSER_DETAILS] WHERE [ID] = " . $this->user_id;
	...
	function lt_report(){
		...
		$conn = sqlsrv_connect( $sys_st->severnam(), $sys_st->connectionInfo());
		if ($conn == true){
			$sql = "SELECT TOP 1 REPORT_TYPE.NAME, REPORT_MAPPING.PROJECT_ID
					FROM REPORT_MAPPING INNER JOIN REPORT_TYPE ON REPORT_MAPPING.REPORT_TYPE_ID = REPORT_TYPE.ID
					WHERE [PROJECT_ID] IN (SELECT [PROJECT_ID] FROM  [PROJECT_MAPPING] WHERE [USER_ID] = '" . $this->user_id . "')
					ORDER BY REPORT_MAPPING.CREATED_DATETIME DESC";
			...
			while($row = sqlsrv_fetch_array($result))
  			{
  				$return_val .= "<h2>" . $row['NAME'] . " </h2>";
				$return_val .= '<a href="' . $int_mg->external_source($this->username . "/" . $row['PROJECT_ID']) . '" class="button-red">View Now</a>';
  			}	
			sqlsrv_free_stmt( $result);
			sqlsrv_close( $conn );
	...
	function cus_det(){
		...
		$conn = sqlsrv_connect( $sys_st->severnam(), $sys_st->connectionInfo());
		if ($conn == true){
			$sql = "SELECT [FIRST_NAME] ,[LAST_NAME] ,[EMAIL] ,[TELEPHONE_LAND] FROM [KNG_ROLE_USERS] WHERE [ID] =" . $this->user_id;
			...
			while($row = sqlsrv_fetch_array($result))
  			{
  				...
  				$return_val .= "<p> " . $row['FIRST_NAME'] . " " . $row['LAST_NAME'] . " </p>";
				$return_val .= "<p><span>e:</span> " . $row['EMAIL'] . "</p>";
				$return_val .= "<p><span>t:</span> " . $row['TELEPHONE_LAND'] . " </p>";
  			}	
			sqlsrv_free_stmt( $result);
			sqlsrv_close( $conn );
	...
	function findeng(){
		...
		$conn = sqlsrv_connect( $sys_st->severnam(), $sys_st->connectionInfo());
		if ($conn == true){
			$sql = "SELECT [USER_ID] FROM [PROJECT_MAPPING] WHERE [ROLE_ID] = 2 AND [PROJECT_ID] = (SELECT [PROJECT_ID]FROM [PROJECT_MAPPING] WHERE [USER_ID] = " . $this->user_id . ")";
			...
			while($row = sqlsrv_fetch_array($result))
  			{
                ...
                $return_val .= "<div class='profile-pic profile-block'>";
                $return_val .= "<img src='" . $img . "'>";
                $return_val .= "</div>";
				$return_val .= "<div class='customer-text profile-block'>";
		
  				$return_val .= "<h1>" . $row['FIRST_NAME'] . " " . $row['LAST_NAME'] ."</h1>";
				$return_val .= "<h4 class='red'>Engineer</h4>";
				$return_val .= "<ul><li><span class='red'>e:</span> " . $row['EMAIL'] ."</li>";
				$return_val .= "<li><span class='red'>t:</span>" . $row['TELEPHONE_LAND'] ."</li>";
				$return_val .= "<li>" . $row['TELEPHONE_MOBILE'] ."</li></ul>";
